...
## Agentic Scale - Track 5
### Propuesta de migración de LocalStorage a persistencia relacional
...
| `scale_agents_news_cache` | Cache de análisis de IA por titular | utilizado para evitar volver a solicitar el análisis al modelo de IA |
...
*(No hay login en el proyecto ni sistema de notificaciones real, así que el esquema no incluye tablas para eso.)*
...
## 4. Propuesta de esquema relacional (compatible con PostgreSQL/Supabase)
...
Este esquema es compatible directamente con PostgreSQL, y con Supabase (que usa PostgreSQL internamente) sin necesidad de adaptaciones adicionales.

---

## Conclusión

El esquema propuesto permite reemplazar el almacenamiento actual basado en `localStorage` por una base de datos relacional, manteniendo la arquitectura existente y facilitando la implementación de un backend utilizando PostgreSQL o Supabase.

Además, normaliza la información, evita duplicación de datos y mejora la integridad referencial mediante el uso de claves foráneas entre noticias, activos, watchlists y briefings.